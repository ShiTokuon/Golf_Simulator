...
namespace Kamgam.Terrain25DLib
{
	...
	public partial class SplineController : MonoBehaviour
	{
        ...
        void onEditorUpdate()
        {
            ...
            {
                // Prevent entering edit mode if the object is a prefab
#if UNITY_EDITOR
                if (EditMode && PrefabUtility.IsPartOfAnyPrefab(gameObject))
                {
                    Debug.LogWarning("Editing prefabs outside of PrefabMode is not supported. Please open the prefab for editing.");
                    EditMode = false; 
                }
#endif
