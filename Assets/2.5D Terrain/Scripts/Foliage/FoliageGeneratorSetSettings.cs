...
namespace Kamgam.Terrain25DLib
{
	...
	public class FoliageGeneratorSetSettings : ScriptableObject
	{
        ...
        [Tooltip("Place only the upside down prefabs.")]
        public bool OnlyUpsideDown = true;
		...
		[Range(-30f, 30f), Tooltip("If objects are not allowed to be placed in the middle then how much are they allowed to overlap the middle coming in from the sides?")]
		public float MiddleWidthOverlapFront = 0f;

        [Range(-30f, 30f), Tooltip("If objects are not allowed to be placed in the middle then how much are they allowed to overlap the middle coming in from the sides?")]
        public float MiddleWidthOverlapBack = 0f;
		...
		[Tooltip("Should the placed objects be scaled randomly? Set to 1 if not.")]
		public float MinScale = 0.5f;

		[Tooltip("Should the placed obects be scaled randomly? Set to 1 if not.")]
		public float MaxScale = 1f;

        [Tooltip("Scale multiplier for each axis. This is applied (via multiplicaton) AFTER the uniform scale by 'MinScale' and 'MaxScale'.")]
        public Vector3 MinScaleXYZ = new Vector3(1f, 1f, 1f);

        [Tooltip("Scale multiplier for each axis. This is applied (via multiplicaton) AFTER the uniform scale by 'MinScale' and 'MaxScale'.")]
        public Vector3 MaxScaleXYZ = new Vector3(1f, 1f, 1f);
		...
		[Tooltip("Should the placed objects be rotated randomly around the x axis?\n\n" +
            "For example: a value of 360 means it will be randomly rotated between -180 and +180 degrees (range of 360).")]
        [Range(0, 180)]
        public float RotXVarianceInDeg = 0f;
        ...
        [Header("Randomness")]
        [Range(0, 1)]
        [Tooltip("Controls the randomness of the rays.\n0 = no randomness, 1 = full randomness")]
